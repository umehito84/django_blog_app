...
class PostListTests(TestCase):
  ...
  def test_get_2posts_by_list(self):
    ...
    # html 内に post1 の title が含まれていることを確認
    self.assertContains(response, 'title1')
    # html 内に post2 の title が含まれていることを確認
...
class PostCreateTests(TestCase):
	...
	def test_get(self):
		"""GETメソッドとしてアクセスしてステータスコード200を返されることを確認"""
	...
	def test_post_null(self):
		"""空のデータで　POST するとリダイレクトもなく　200だけ返されることを確認"""
		data = {}
		response = self.client.post(reverse('blog:post_create'), data=data)
		self.assertEqual(response.status_code, 200)

class PostDetailTests(TestCase):
	"""PostDetailViewのテストクラス"""

	def test_not_fount_pk_get(self):
		"""記事を登録せず、空の状態で存在しない記事のプライマリーでアクセスした時 404 が返されることを確認"""
...
class PostDeleteTests(TestCase):
	"""PostDeleteViewのテストクラス"""

	def test_not_fount_pk_get(self):
		"""記事を登録セず、空の状態で存在しない記事のプライマリーキーでアクセス"""
		response = self.client.get(
